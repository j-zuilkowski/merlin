...
model-ID preflight path and critic failure summaries under focused tests, but
the release gate is still red until `scripts/release/run-capability-gate.sh`
passes both S1 and S2 in one deterministic run. The latest live evidence timed
out in S1 and left S2 red on `add_rejects_non_numeric_amount`. Gate #10, the
KiCad release screenshot step, is not valid until gates #1-#9 are green.
