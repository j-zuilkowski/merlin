...
Task 322 complete: dead TelemetryEmitter setters removed.
...
1. **`TaskScanner` reads the wrong doc tier.** `extractDeclaredSurfaces` only reads
   files matching `task-\d+b-`. But the "New surface introduced in task" block lives
...
Task 323b fixes both: `TaskScanner` reads all task docs (`a`, `b`, `diag-*`); the
engine surfaces `.red`/`.yellow`/`.orange` drift, all as `.nudge` (task drift is
advisory, never a commit-blocker).

**This is a runtime-failure task.** The tests compile against existing APIs and FAIL
...
TDD coverage:
  ...
  File 2 — DisciplineEngineTaskDriftSeverityTests: drift findings are nudge, not block;
...
## Write to: MerlinTests/Unit/DisciplineEngineTaskDriftSeverityTests.swift

```swift
import XCTest
@testable import Merlin

/// Task 323a — failing test: DisciplineEngine must surface taskDrift findings as
...
final class DisciplineEngineTaskDriftSeverityTests: XCTestCase {
    ...
    func testTaskDriftFindingsAreNudgeAndYellowIsSurfaced() async throws {
        ...
        // A `b` task doc (read even before task 323) declaring two surfaces:
...
xcodebuild -scheme MerlinTests test -destination 'platform=macOS' \
  ...
  -only-testing:MerlinTests/DisciplineEngineTaskDriftSeverityTests 2>&1 \
  | grep -E 'Test Case|TEST (SUCCEEDED|FAILED)|error:'
```
Expected: BUILD SUCCEEDED; `testReadsNewSurfaceBlockFromTestsTaskDoc`,
`testReadsNewSurfaceBlockFromDiagTaskDoc`, and
`testTaskDriftFindingsAreNudgeAndYellowIsSurfaced` all **FAIL** against today's
...
git add MerlinTests/Unit/TaskScannerDocCoverageTests.swift \
  ...
  tasks/task-323a- taskscanner-doc-coverage-tests.md
git commit -m "Task 323a — TaskScanner doc-coverage & drift-severity tests (failing)"
```
