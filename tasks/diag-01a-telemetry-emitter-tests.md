...
New surface introduced in phase diag-01b:
  ...
  - `TelemetryEmitter` — `@unchecked Sendable` singleton; fire-and-forget `emit(_:durationMs:data:)`;
     writes JSON lines to `~/.merlin/telemetry.jsonl` on a background serial queue;
     rotates file at 10 MB (renames to `telemetry.1.jsonl`);
     exposes `resetForTesting(path:)` to redirect output in tests

TDD coverage:
  File 1 — TelemetryEmitterTests: event encoding, span timing, file write, file rotation, graceful-no-op when path unwritable

---

## Write to: MerlinTests/Unit/TelemetryEmitterTests.swift
...
```
Expected: BUILD FAILED — `TelemetryEmitter`, `TelemetryEvent`, `TelemetryValue`, `TelemetrySpan` not found.
