...
New surface introduced in phase 35b:
  ...
  - `StagingBuffer.commentsAsAgentMessage([UUID])` — serialises all comments across listed
    change IDs into a single follow-up message string for the agent
  - `AgenticEngine.submitDiffComments(changeIDs:)` — calls commentsAsAgentMessage and
    injects it as a user turn, triggering a new loop iteration

TDD coverage:
  File 1 — DiffCommentTests: addComment appends; commentsAsAgentMessage formats correctly;
            submitDiffComments injects a message into the engine context

---

## Write to: MerlinTests/Unit/DiffCommentTests.swift
...
Expected: `BUILD FAILED` with errors referencing `DiffComment`,
`StagingBuffer.addComment(_:toChange:)`, `StagingBuffer.commentsAsAgentMessage`,
`AgenticEngine.submitDiffComments`.
