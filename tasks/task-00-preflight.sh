...
# Simulator runtime (needed for task-08b, task-23)
...
# xcresulttool (needed for task-08b xcresult parsing)
...
if command -v codex &>/dev/null; then
    ...
    warn "  Alternatively use the Codex GUI and paste task file contents manually."
fi
...
" 2>/dev/null)
    ...
    warn "  Required for: task-09b (screen capture), task-10 (vision query), task-23/24 (E2E)."
...
if [ -f "$proj_dir/spec.md" ]; then
    ok "spec.md present"
else
    warn "spec.md missing — Codex context will be incomplete."
fi
...
if [ "$errors" -eq 0 ] && [ "$warnings" -eq 0 ]; then
    ...
    echo ""
    echo "  Fix all errors, then re-run: bash tasks/task-00-preflight.sh"
fi
