...
This is a **shipping-bug fix** (single task file — no NNa/NNb pair).
...
1. The 5 `project:*` skill folders are added to `Merlin/Skills/Builtin/`, each as
   ...
   version-controlled instead of living only in `~/.merlin/` and in task files
   259b–263b.
...
```bash
git add tasks/task-281-bundle-builtin-skills.md \
    project.yml \
    Merlin.xcodeproj/project.pbxproj \
    Merlin/Skills/Builtin/project-init/SKILL.md \
    Merlin/Skills/Builtin/project-task/SKILL.md \
