...
New surface introduced in task 164b:
  ...
  - AgenticEngine retry loop — on critic `.fail`, injects correction system message and
    re-runs the while loop (up to `maxCriticRetries` times per turn)
  - `OutcomeSignals.criticRetryCount` — populated from actual retries (was hardcoded 0)
  - `OutcomeSignals.stage1Passed` — `true` when critic passes, `false` when retries
    exhausted on fail, `nil` when skipped (was hardcoded nil)

TDD coverage:
  File 1 — AgenticEngineCriticRetryTests: AppSettings defaults, retry count, correction
    injection, escalation note after exhaustion, OutcomeSignals wiring, criticEnabled guard

---

## Write to: MerlinTests/Unit/AgenticEngineCriticRetryTests.swift
...
actor CapturingPerformanceTracker: ModelPerformanceTrackerProtocol {
    nonisolated(unsafe) var lastSignals: OutcomeSignals?
...
```
Expected: BUILD FAILED with errors naming `criticEnabled`, `maxCriticRetries` on `AppSettings`.
