...
- **Version banners — update every doc that carries one** (2.2.1 → 2.2.2, build 18 → 19):
    - `constitution.md`: `**Current version: 2.2.1** (build 18, tag v2.2.1)` →
...
grep -rnE "2\.2\.1|build 18|Version 2\.0|2\.0\.0" \
    README.md constitution.md Requirements.md Merlin/Docs/UserGuide.md Merlin/Docs/DeveloperManual.md
```

Expected: **no output** except, in `constitution.md`, the historical "Versioning" example text
...
git add tasks/task-278b-v2-2-2-release.md \
    project.yml \
    constitution.md \
    README.md \
    Requirements.md \
