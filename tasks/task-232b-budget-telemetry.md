...
Task 232a complete: failing tests are in place for redacted error-body capture,
pre-flight estimate emit, and planner step trace.

Pure observability task — no user-visible behaviour change. The new events feed the calibration
of every later task in the 232–240 series. `engine.preflight.estimate` here uses the existing
rough estimator; task 233b promotes that to the formal pre-flight gate.
...
Expected: **BUILD SUCCEEDED** and all task 232a tests pass. No prior tests regress.
...
git commit -m "Task 232b — Budget telemetry (error body, pre-flight estimate, planner step trace)"
```

## PASTE-LIST update

Append the two new  tasks to `tasks/PASTE-LIST.md` under a new "Budget-Aware Execution (v2.1.0)" section.
