# Single-Shot Codex Prompt — Build All Diag Tasks
...
- Zero warnings, zero errors after every task
- TDD: task NNa writes failing tests, NNb makes them pass — commit after each task
- Never use git add -A; always add specific files
- Never skip the commit step

Execute ALL of the following  tasks in order. For each task:
  1. Read the task file from tasks/
  2. Write/edit exactly the files listed in that task
  3. Run the verify command and confirm the expected result
  4. Run the commit command

---

TASK diag-01a — Read tasks/diag-01a-telemetry-emitter-tests.md
...
Commit: git add MerlinTests/Unit/TelemetryEmitterTests.swift && git commit -m "Task diag-01a — TelemetryEmitter tests (failing)"

TASK diag-01b — Read tasks/diag-01b-telemetry-emitter.md
...
Commit: git add Merlin/Telemetry/TelemetryEmitter.swift project.yml Merlin.xcodeproj && git commit -m "Task diag-01b — TelemetryEmitter core"

TASK diag-02a — Read tasks/diag-02a-provider-telemetry-tests.md
Write: MerlinTests/Unit/ProviderTelemetryTests.swift
Verify: xcodebuild -scheme MerlinTests build-for-testing -destination 'platform=macOS' -derivedDataPath /tmp/merlin-derived 2>&1 | grep -E 'error:|warning:|BUILD SUCCEEDED|BUILD FAILED' | head -40
Expected: BUILD FAILED (session: param missing, telemetry events absent)
Commit: git add MerlinTests/Unit/ProviderTelemetryTests.swift && git commit -m "Task diag-02a — Provider telemetry tests (failing)"

TASK diag-02b — Read tasks/diag-02b-provider-telemetry.md
...
Commit: git add Merlin/Providers/OpenAICompatibleProvider.swift Merlin/Providers/DeepSeekProvider.swift && git commit -m "Task diag-02b — Provider telemetry instrumentation"

TASK diag-03a — Read tasks/diag-03a-engine-telemetry-tests.md
...
Commit: git add MerlinTests/Unit/EngineTelemetryTests.swift && git commit -m "Task diag-03a — Engine telemetry tests (failing)"

TASK diag-03b — Read tasks/diag-03b-engine-telemetry.md
...
Commit: git add Merlin/Engine/AgenticEngine.swift && git commit -m "Task diag-03b — Engine telemetry instrumentation"

TASK diag-04a — Read tasks/diag-04a-memory-telemetry-tests.md
...
Commit: git add MerlinTests/Unit/MemoryTelemetryTests.swift MerlinTests/Unit/RAGTelemetryTests.swift && git commit -m "Task diag-04a — Memory & RAG telemetry tests (failing)"

TASK diag-04b — Read tasks/diag-04b-memory-telemetry.md
...
Commit: git add Merlin/Memories/MemoryEngine.swift Merlin/RAG/XcalibreClient.swift && git commit -m "Task diag-04b — Memory & RAG telemetry instrumentation"

TASK diag-05a — Read tasks/diag-05a-context-planner-critic-tests.md
...
Commit: git add MerlinTests/Unit/ContextCompactionTelemetryTests.swift MerlinTests/Unit/PlannerTelemetryTests.swift MerlinTests/Unit/CriticTelemetryTests.swift && git commit -m "Task diag-05a — Context, planner & critic telemetry tests (failing)"

TASK diag-05b — Read tasks/diag-05b-context-planner-critic.md
...
Commit: git add Merlin/Engine/ContextManager.swift Merlin/Engine/PlannerEngine.swift Merlin/Engine/CriticEngine.swift && git commit -m "Task diag-05b — Context, planner & critic telemetry instrumentation"

TASK diag-06a — Read tasks/diag-06a-infra-telemetry-tests.md
...
Commit: git add MerlinTests/Unit/SessionStoreTelemetryTests.swift MerlinTests/Unit/HookTelemetryTests.swift MerlinTests/Unit/ProcessMemoryTelemetryTests.swift && git commit -m "Task diag-06a — Infrastructure telemetry tests (failing)"

TASK diag-06b — Read tasks/diag-06b-infra-telemetry.md
...
Expected: BUILD SUCCEEDED, all new tests pass
Commit: git add Merlin/Telemetry/TelemetryEmitter.swift Merlin/Sessions/SessionStore.swift Merlin/Hooks/HookEngine.swift Merlin/MCP/MCPBridge.swift && git commit -m "Task diag-06b — Infrastructure telemetry instrumentation"

TASK diag-07a — Read tasks/diag-07a-accessibility-tests.md
...
Commit: git add MerlinTests/Unit/AccessibilityIDTests.swift MerlinTests/Unit/GUIActionTelemetryTests.swift && git commit -m "Task diag-07a — Accessibility identifier tests (failing)"

TASK diag-07b — Read tasks/diag-07b-accessibility.md
...
Commit: git add Merlin/Support/AccessibilityID.swift Merlin/Telemetry/TelemetryEmitter.swift Merlin/Views/ChatView.swift Merlin/Views/SessionSidebar.swift Merlin/Views/ProviderHUD.swift Merlin/Views/ContentView.swift && git commit -m "Task diag-07b — Accessibility identifiers and GUI action telemetry"
...
If any task fails, fix it before proceeding — do not skip  tasks.
```
