# Single-Shot Codex Prompt — W4 Trace-Audit Tasks 320–321
...
```
You are executing trace-audit fix  tasks for the Merlin macOS SwiftUI app.
Working directory: ~/Documents/localProject/merlin

Rules (non-negotiable):
- Swift 5.10, macOS 14+, SWIFT_STRICT_CONCURRENCY=complete
- Zero warnings, zero errors after every NNb task
- TDD: task NNa writes failing tests, NNb makes them pass — commit after EACH task
- Never use git add -A; add only the specific files named in the task's Commit step
...
Execute the following four  tasks STRICTLY IN ORDER. For each task:
  1. Read the task file from tasks/
  2. Write/edit exactly the files in that task's "Write to:" / "Edit:" section
  3. Run `xcodegen generate` (a new MerlinTests file must be added to the project)
  4. Run the task's Verify command and CONFIRM the expected result before continuing
  5. Run the task's Commit command
If a Verify result does not match Expected, STOP and report — do not proceed.

---

TASK 320a — Read tasks/task-320a-worker-diff-actions-tests.md
Write: MerlinTests/Unit/WorkerDiffViewActionTests.swift
Kind: COMPILE-FAILURE task — verify with build-for-testing.
...
TASK 320b — Read tasks/task-320b-worker-diff-actions.md
...
Kind: IMPLEMENTATION task — verify with test, then build-for-testing on BOTH
...
TASK 321a — Read tasks/task-321a-doc-reference-comment-tests.md
Write: MerlinTests/Unit/DocReferenceGraphCommentTests.swift
Kind: RUNTIME-FAILURE task — verify with test (build succeeds, the test fails).
...
TASK 321b — Read tasks/task-321b-doc-reference-comment.md
...
Rewrite: MerlinTests/Unit/DocReferenceDanglingTests.swift  (§2 — onto fenced-block
         fixtures; repairs test rot left by task 319b)
Kind: IMPLEMENTATION task — verify with test (all seven DocReference* classes),
