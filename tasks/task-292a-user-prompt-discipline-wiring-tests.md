...
New behaviour in task 292b (no new public API — pure wiring):
...
TDD coverage:
  ...
  no task file emits the note; a bug-fix prompt does not.
...
Expected: BUILD SUCCEEDED, `testFeatureRequestWithoutTaskFileEmitsDisciplineNote` FAILS
(no note emitted yet); `testBugFixPromptEmitsNoDisciplineNote` passes.

## Commit
git add MerlinTests/Unit/UserPromptDisciplineWiringTests.swift tasks/task-292a-user-prompt-discipline-wiring-tests.md
git commit -m "Task 292a — User-prompt discipline wiring tests (failing)"
