...
New surface introduced in phase 130b:
  ...
  `CalibrationAdvisor` — struct (Sendable);
      func analyze(responses:localModelID:localProviderID:) -> [ParameterAdvisory]
      func categoryBreakdown(responses:) -> [CalibrationCategory: CategoryScores]
    Detection thresholds:
      - overallDelta ≥ 0.40 → .contextLengthTooSmall
      - local score std-dev ≥ 0.22 across responses → .temperatureUnstable
      - ≥ 50% of responses have localResponse/referenceResponse length ratio < 0.30 → .maxTokensTooLow
      - ≥ 50% of local responses have trigram repetition ratio > 0.45 → .repetitiveOutput
      - overallDelta < 0.15 → no advisory

TDD coverage:
  File 1 — MerlinTests/Unit/CalibrationAdvisorTests.swift

---

## Write to: MerlinTests/Unit/CalibrationAdvisorTests.swift
...
```
Expected: **BUILD FAILED** — `CalibrationAdvisor`, `CategoryScores` not defined.
