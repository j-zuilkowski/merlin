...
New surface introduced in phase 44b:
  ...
  - `TOMLDecoder` — `decode<T: Decodable>(_ type: T.Type, from string: String) throws -> T`
  - `TOMLValue.toFoundation() -> Any` — bridge to Foundation JSON pipeline

TDD coverage:
  File 1 — TOMLDecoderTests: full TOML spec coverage — primitives, tables, arrays, array-of-tables,
           dotted keys, inline tables, multi-line strings, escape sequences, comments, Codable round-trips,
           error cases

---

## Write to: MerlinTests/Unit/TOMLDecoderTests.swift
...
```
Expected: BUILD FAILED with errors for `TOMLDecoder`, `TOMLValue`, `TOMLParser` — not yet defined.
