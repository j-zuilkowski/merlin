# Task 259a — project:init Skill Tests
...
Task 258b complete: OverrideAuditLog + weekly review event live.

Skills ( tasks 259–263) are SKILL.md files written to `~/.merlin/skills/project-*/SKILL.md`.
The `a` task asserts the skill file does not yet exist (BUILD FAILED = skill file absent).
The `b` task writes the SKILL.md content.

For task 259a, the "test" is a Swift test that asserts the presence of
...
New surface introduced in task 259b:
...
```bash
git add tasks/task-259a-project-init-skill-tests.md \
    MerlinTests/Unit/ProjectInitSkillTests.swift
git commit -m "Task 259a — ProjectInitSkillTests (failing)"
```
