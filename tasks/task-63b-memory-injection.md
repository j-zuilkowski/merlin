...
## Edit: Merlin/Engine/ConstitutionLoader.swift

Add the following static methods to the `ConstitutionLoader` enum (before the closing brace):
...
Add `memoriesContent` property after `constitutionContent`:

```swift
    var memoriesContent: String = ""
```

Update `buildSystemPrompt()` to include the memories block after constitution.md content:
...
```swift
        appState.engine.memoriesContent = ConstitutionLoader.defaultMemoriesBlock()
```
