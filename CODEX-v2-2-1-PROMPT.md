...
Discipline Subsystem. Read constitution.md first. Then execute these  tasks in strict order,
committing after each one, without stopping between  tasks:
...
- TDD: every NNa ends with a commit of just the failing tests. Every NNb ends with
  ...
  Never --no-verify.
- NNa expected outcome VARIES per task — each task file states its own expectation in
  the ## Verify section. Some NNa  tasks are BUILD FAILED (new symbol referenced); others
...
- Per-task loop: cat the task file → implement → verify against the file's stated
  expectation → commit → next task.
- Some  tasks update pre-existing tests (269b updates AdapterSeedTests). The task file
...
- Stop and report only if: verify outcome does not match the task file's stated
  expectation, a pre-existing test unrelated to the task regresses, or a git commit is
  rejected by a hook.
