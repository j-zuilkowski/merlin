...
Discipline Subsystem. Read CLAUDE.md first. Then execute these phases in strict order,
...
- TDD: every NNa ends with a commit of just the failing tests. Every NNb ends with
  ...
  Never --no-verify.
- NNa expected outcome VARIES per phase — each phase file states its own expectation in
  ...
  an existing symbol). Trust the phase file's stated expectation, not a blanket rule.
- CODE SIGNING: this sandbox has no signing cert. Every xcodebuild call must include:
    CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO
  The phase files already include these flags. Do not remove them.
- Per-phase loop: cat the phase file → implement → verify against the file's stated
  expectation → commit → next phase.
- Some phases update pre-existing tests (269b updates AdapterSeedTests). The phase file
  names those files — that is expected, not a regression.
- Stop and report only if: verify outcome does not match the phase file's stated
...
cat phases/phase-266a-finding-dedup-key-tests.md
```
