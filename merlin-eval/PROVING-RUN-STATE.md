...
only authors phase docs for Codex. **Never `git push`** — local commits only.
...
- Working dir: `~/Documents/localProject/merlin` (git repo). `merlin-eval/` is now
  *inside* it at `merlin/merlin-eval/` (relocated in phase 332).
...
- **Phase-doc sync deferred:** per `CLAUDE.md`, the Merlin fixes need a `## Fixes` note
  in the relevant phase docs (`ShellTool`, `LoRATrainer`, `EvalSupport`, the
...
6. Write the final report; do the deferred phase-doc `## Fixes` sync.
