...
**[v2.0]** Electronics/KiCad feature set: `plugins/electronics`, raster/PDF schematic ingestion, KiCad project generation, FreeRouting-backed route loop, ERC/DRC/parity/SPICE/fab gates, vendor-native BOM/order workflows.
...
│  SchedulerEngine [v2]         PRMonitor [v2]         ConstitutionLoader [v2]     │
│  ConnectorsLayer [v2]                                                        │
├──────────────────────────────────────────────────────────────────────────────┤
│                           Provider Layer [v1]                                │
│                                                                              │
...
## llama.cpp First-Class Local Provider [v2.3]
...
## CAG — Cache-Augmented Generation [v11]

### Shipped runtime surface

CAG is now implemented as a request policy and deterministic prefix discipline, not a separate provider mode:
...
- **Hot system block**: unpinned constitution.md and per-turn system warnings.
...
AgenticEngine.send(userMessage:)
  ...
  └── provider.complete(request:)
         ├── AnthropicProvider emits explicit cache_control markers
         └── Other providers rely on stable prefix bytes only
```

### Metrics

`AnthropicSSEParser` maps:

- `usage.cache_read_input_tokens` -> `CAGCacheUsage.readTokens`
- `usage.cache_creation_input_tokens` -> `CAGCacheUsage.creationTokens`
...
- constitution.md loaded from the worktree root at session creation
...
## constitution.md Loader [v2]

At session creation, `ConstitutionLoader` searches for instruction files from the project root upward:

```
<project-root>/constitution.md
<project-root>/.merlin/constitution.md
~/constitution.md
```
...
user opens Settings > Memories > pending queue
    → reviews / edits / accepts or rejects
        ...
        → rejected: deleted
next session init (ConstitutionLoader)
    → reads ~/.merlin/memories/*.md
    → injects as second system prompt block after constitution.md content
```
...
├── Engine/
│   ├── AgenticEngine.swift
│   ├── ConstitutionLoader.swift          [v2] — searches 3 paths; injects at session init
...
**[v2] Layer 2 additions:** StagingBuffer accept/reject, WorktreeManager create/remove, ConstitutionLoader discovery, SkillsRegistry loading and frontmatter parsing, MCPBridge tool registration.
...
| constitution.md | Not supported | Auto-loaded at session init |
...
2. Missing `plugins/electronics` route or missing required tool returns `BLOCKED_TOOLING`.
...
> **V6 — not in V5 scope.** Requires Unsloth integration exploration and LM Studio adapter loading investigation before task files can be written. The `ModelPerformanceTracker` (V5) already collects the training corpus as a byproduct — no tracker changes needed in V6.
...
But construction discipline — TDD task pairs, version bumps at the right moment, documentation
...
│   /project:task    — build a TDD task pair                             │
...
<project>/
  ...
  tasks/                       — Merlin's existing convention; not changed
  docs/                         — optional alternative location for the doc set
```

### TaskScanner

Reads `tasks/` directory, extracts declared surfaces from each NNb file, cross-checks against
...
| `constitution.md`, `AGENTS.md`, task files | no constraint |
...
- `constitution.md` from template, customised for chosen language and adapter
- Doc set from templates (`README.md`, `spec.md`, `api.md`, `developer-guide.md`,
  `user-manual.md`, `FEATURES.md`, `CHANGELOG.md`)
- `tasks/` directory with `task-00-scaffold.md` documenting the initial state
...
□ TaskScanner reports clean (no red/orange drift)
...
6. Produces a one-page report: *"Adopted. Baseline coverage gap: 314 surfaces. Default
   ...
   Prose readability fails in 6 docs. Task drift: green except 3 yellow."*
...
| `UserPromptSubmit` | Before user message processed | If prompt looks like a feature request, check that an appropriate task file exists |
| `PostCommit` (new) | After `git commit` succeeds | Run `TaskScanner` if the commit touched source files |
...
| `TaskScanner` | Swift actor inside `DisciplineEngine` | `scan.sh` bash script |
...
- **Implementation surface is significant** (~24 task pairs). Mitigation: each pair is small
