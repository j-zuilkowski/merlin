...
Then execute these  tasks in strict order, committing after each one:
...
- Per-task loop: cat the task file → implement → verify (expected outcome is in the file)
  → commit → next task.
- Stop and report if: verify outcome doesn't match expectation, any pre-existing test
  regresses, git commit is rejected, or a surface named in a later task already exists.
