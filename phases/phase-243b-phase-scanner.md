# Phase 243b — PhaseScanner

## Context
Swift 5.10, macOS 14+, SwiftUI + async/await. Non-sandboxed. No third-party packages.
SWIFT_STRICT_CONCURRENCY=complete. Zero warnings, zero errors required.
Working dir: ~/Documents/localProject/merlin
Phase 243a complete: failing tests for DriftSeverity, DriftFinding, and PhaseScanner.
...
### Merlin/Discipline/PhaseScanner.swift (new file)
...
actor PhaseScanner {
    ...
    func scan(projectPath: String) async -> [DriftFinding] {
        ...
        // 1. Extract declared surfaces from NNb phase files
    ...
    private func extractDeclaredSurfaces(
        ...
        let nnbFiles = files.filter { file in
            ...
            // Match phase-NNb-* files (letter b after digits)
            return name.hasPrefix("phase-") &&
                   name.hasSuffix(".md") &&
                   file.lastPathComponent.range(
                       of: #"phase-\d+b-"#, options: .regularExpression) != nil
        }
        ...
        for file in nnbFiles {
            ...
            result.append(contentsOf: surfaces.map { ($0, phaseID) })
        }
        return result
    }

    private func extractPhaseID(from filename: String) -> String {
        // "phase-233b-provider-budget.md" -> "233b"
    ...
    private func enumerateSourceSymbols(root: URL) -> [String] {
        ...
        // Exclude phases/ and tests/ directories
        for case let url as URL in enumerator {
            let path = url.path
            if path.contains("/phases/") || path.contains("Tests/") { continue }
...
```bash
git add phases/phase-243b-phase-scanner.md \
    Merlin/Discipline/DriftFinding.swift \
    Merlin/Discipline/PhaseScanner.swift
git commit -m "Phase 243b — PhaseScanner + DriftFinding + DriftSeverity"
```
