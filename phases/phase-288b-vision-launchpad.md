...
— `vision.md → architecture.md → phases/ → code`. `project:init` seeds it with the
...
- Replace the opening paragraphs with a brief statement that `vision.md` is the idea
  launchpad — ideas land in `## Active`, are promoted to `architecture.md`, or are
  parked in `## Deferred`; `architecture.md` remains the source of truth for committed
  design.
...
```bash
git add phases/phase-288a-vision-launchpad-tests.md \
    phases/phase-288b-vision-launchpad.md \
...
is `vision.md → architecture.md → phases/ → code`. This repo's own `vision.md` is
