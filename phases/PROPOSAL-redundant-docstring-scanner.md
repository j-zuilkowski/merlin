# Proposal — RedundantDocstringScanner (next phase pair: 333a + 333b)

**Status:** Draft proposal. Not yet committed; awaiting user approval to convert to phase-333a (tests) + phase-333b (implementation).

## Motivation

The 2026-05-20 comment audit found ~120–180 instances of doc-comments that restate the symbol name without adding information (e.g. `/// The text content of the memory.` on `let content: String`). These directly contradict the project's stated comment standard in [CLAUDE.md](../CLAUDE.md):
...
This proposal is intentionally not started. Convert to `phase-333a-redundant-docstring-scanner-tests.md` + `phase-333b-redundant-docstring-scanner.md` when ready to commission.
