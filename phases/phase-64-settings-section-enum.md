...
Phase 63b complete: memory injection wired through CLAUDEMDLoader + AgenticEngine.
...
Replace the `SettingsSection` enum and the `detailView(for:)` switch in `SettingsWindowView`.
The stubs `AgentSettingsView`, `MCPSettingsView`, `SkillsSettingsView`, `SearchSettingsView`,
and `PermissionsSettingsView` are added as private structs at the bottom of the file.
Remove `ShortcutsSettingsView`.
