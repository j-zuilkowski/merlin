...
New surface introduced in phase 32b:
  ...
  - `StagingBuffer` — actor: stage(_:), accept(_:), reject(_:), acceptAll(), rejectAll(),
    pendingChanges: [StagedChange]
  - `ToolRouter.stagingBuffer: StagingBuffer?` — when set, file-write tools in ask/plan
    mode are intercepted and staged rather than executed
  - `ToolRouter.permissionMode: PermissionMode` — checked on each file-write dispatch

TDD coverage:
  File 1 — StagingBufferTests: stage appends; accept writes file + removes from pending;
            reject removes without writing; acceptAll/rejectAll operate on all changes

---

## Write to: MerlinTests/Unit/StagingBufferTests.swift
...
Expected: `BUILD FAILED` with errors referencing `StagingBuffer`, `StagedChange`, `ChangeKind`.
