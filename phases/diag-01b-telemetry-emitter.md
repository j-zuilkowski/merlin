...
## Write to: Merlin/Telemetry/TelemetryEmitter.swift
...
## Add to project.yml (sources for Merlin target)

Ensure `Merlin/Telemetry` is included. Add under the Merlin target sources if not already covered by a glob:

```yaml
# In project.yml under targets: Merlin: sources:
- Merlin/Telemetry
```
...
xcodebuild -scheme MerlinTests test \
    ...
    | grep -E 'TelemetryEmitter|passed|failed|BUILD SUCCEEDED|BUILD FAILED' | head -40
```
Expected: all TelemetryEmitterTests pass, BUILD SUCCEEDED.
