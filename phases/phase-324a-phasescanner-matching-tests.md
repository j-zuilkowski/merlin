...
Phase 323b complete: `PhaseScanner` now reads all phase docs.

W4 trace audit finding F8 (surfaced by phase 323). Once `PhaseScanner` could see all
~200 phase docs, the scan jumped 211 → 981 findings — but ~900 are false positives from
crude symbol matching, not real drift:
- **~400** — phase docs declare members qualified (`AgenticEngine.invokeSkill(_:)`); the
  scanner stores only the bare `invokeSkill` from source, so the names never match.
- **~10** — enum cases declared as `.fail`; `PhaseScanner` never enumerates `case`
...
Phase 324b fixes `PhaseScanner` matching: `canonicalDeclaration` strips declaration-kind
...
`PhaseScanner.scan` API and FAIL at runtime against today's matching. Verify with `test`.

TDD coverage: `MerlinTests/Unit/PhaseScannerMatchingTests.swift` — qualified member,
...
## Write to: MerlinTests/Unit/PhaseScannerMatchingTests.swift
...
xcodebuild -scheme MerlinTests test -destination 'platform=macOS' \
  ...
  -only-testing:MerlinTests/PhaseScannerMatchingTests 2>&1 \
...
## Commit
```
git add MerlinTests/Unit/PhaseScannerMatchingTests.swift phases/phase-324a-phasescanner-matching-tests.md
git commit -m "Phase 324a — PhaseScanner symbol-matching tests (failing)"
```
