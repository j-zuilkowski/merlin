...
This phase fixes the test-file exclusion in `PhaseScanner`, the comment/string-literal
false positives in `WhyCommentScanner`, and confirms the per-section reference
association in `DocReferenceGraph.build()`.

---

## Edit: Merlin/Discipline/PhaseScanner.swift
...
- `PhaseScanner.enumerateSourceDeclarations` now excludes any file whose path has a
...
```bash
git add phases/phase-268b-scanner-accuracy.md \
