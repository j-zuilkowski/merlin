...
**Bug A — `PhaseScanner` test-file filter.** `PhaseScanner.enumerateSourceDeclarations`
...
New surface introduced in phase 268b:
  - `PhaseScanner` source enumeration excludes any file whose path has a component
...
TDD coverage:
  File 1 — `PhaseScannerTestExclusionTests.swift`: a temp project with a phase file and
    a `MerlinTests/Unit/Foo.swift` containing `public func testThing()` → `PhaseScanner`
...
## Write to: MerlinTests/Unit/PhaseScannerTestExclusionTests.swift
...
`PhaseScannerTestExclusionTests`, `WhyCommentFalsePositiveTests`, and
...
```bash
git add phases/phase-268a-scanner-accuracy-tests.md \
    MerlinTests/Unit/PhaseScannerTestExclusionTests.swift \
