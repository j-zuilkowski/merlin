...
After the existing `var claudeMDContent: String = ""` property, add:
...
### 8. Add `refreshDistilledClaudeMD(using:)` async method
...
observer in `AppState` (or wherever `engine.claudeMDContent` is set) that sets
...
but since `_stablePrefixDirty = true` is set by `didSet` on `claudeMDContent`, the cache will be
invalidated anyway the next time CLAUDE.md reloads. For Settings toggle take effect immediately,
...
Manual verification:
1. Open a project with a large `CLAUDE.md`.
2. Enable **Settings → Agent → Prompt Compression**.
3. Start a session — observe a one-time distillation delay (provider call).
4. Subsequent turns in the same session use the cached distilled CLAUDE.md immediately.
5. Disable the toggle — the next session reverts to the full original CLAUDE.md.
6. Edit `CLAUDE.md` on disk — the next session detects the hash change and re-distils.
...
git commit -m "Phase 207b — instruction distillation: distilled core prompt + cached CLAUDE.md compression"
```
