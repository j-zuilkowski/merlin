...
`architecture.md` — there is no artifact upstream of it, so a raw idea has nowhere to
...
to `architecture.md`. It is a single document with two sections:
  - **## Active** — ideas captured, awaiting promotion to `architecture.md`.
  - **## Deferred** — ideas consciously parked, each with a "reconsider when".

`architecture.md` remains the source of truth for *what is being built*; `vision.md` is
