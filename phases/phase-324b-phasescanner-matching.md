...
Phase 324a complete: failing tests in `PhaseScannerMatchingTests`.

W4 trace audit finding F8. Four edits to `Merlin/Discipline/PhaseScanner.swift` make its
...
## 1. Edit: Merlin/Discipline/PhaseScanner.swift — `scan` (collapse green/yellow/red)
...
## 2. Edit: Merlin/Discipline/PhaseScanner.swift — `canonicalDeclaration`
...
## 3. Edit: Merlin/Discipline/PhaseScanner.swift — `extractSurfaces` + new helper
...
## 4. Edit: Merlin/Discipline/PhaseScanner.swift — enumerate enum cases
...
```
Expected: the full `MerlinTests` suite passes — including `PhaseScannerMatchingTests`
...
phase-doc fixture. **If a failure appears in a test unrelated to `PhaseScanner` /
...
## Commit
```
git add Merlin/Discipline/PhaseScanner.swift \
  MerlinTests/Unit/DisciplineEnginePhaseDriftSeverityTests.swift \
  phases/phase-324b-phasescanner-matching.md
git commit -m "Phase 324b — PhaseScanner symbol-matching accuracy"
```
