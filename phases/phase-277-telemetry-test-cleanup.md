...
This is a **cleanup phase** (no new behaviour, single phase file — no NNa/NNb pair).
...
4. Four phase docs still describe the removed `TelemetryRecorder` / `sink` seam.
...
Four phase docs still describe the removed seam. Update each:

- `phases/phase-232a-budget-telemetry-tests.md` and
  `phases/phase-232b-budget-telemetry.md` — these introduce `TelemetryRecorder` /
...
- `phases/phase-237a-executor-gate-tests.md` — the prose "reading it back via
  `TelemetryRecorder`" must be updated to "reading it back from the telemetry JSONL file
  via `readTelemetryEvents(fromFile:)`".
- `phases/phase-245a-discipline-engine-tests.md` — the embedded test code uses
...
- Then run a final check and fix anything missed:
  ...
  (`phase-277-telemetry-test-cleanup.md`) and the superseded banners.
- `phases/PASTE-LIST.md` — append phase 277 under the Project Discipline section.
...
```bash
git add phases/phase-277-telemetry-test-cleanup.md \
    Merlin/Telemetry/TelemetryEmitter.swift \
    TestHelpers/TelemetryTestSupport.swift \
    Merlin.xcodeproj/project.pbxproj \
    phases/phase-232a-budget-telemetry-tests.md \
    phases/phase-232b-budget-telemetry.md \
    phases/phase-237a-executor-gate-tests.md \
    phases/phase-245a-discipline-engine-tests.md \
    phases/PASTE-LIST.md \
...
the queue is empty). Updates the four phase docs that referenced the removed seam.
