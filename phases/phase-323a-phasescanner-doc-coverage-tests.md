...
W4 trace audit finding F4. Two linked defects in the phase-drift scanner:

1. **`PhaseScanner` reads the wrong doc tier.** `extractDeclaredSurfaces` only reads
   files matching `phase-\d+b-`. But the "New surface introduced in phase" block lives
   in the `a` (tests) doc per the project template — 200 of 263 `phase-*a-*.md` carry it
   versus 8 of 266 `phase-*b-*.md`. The `diag-*` series is excluded entirely. So the
   scanner sees almost no declared surface and flags ~210 `public` symbols as orange
   drift that are in fact phase-documented.
...
2. **`DisciplineEngine` maps red drift to a commit-block.** `DisciplineEngine.scan`
   ...
   fixed the scanner reads ~200 historical phase docs and finds genuine `.red` (symbol
...
Phase 323b fixes both: `PhaseScanner` reads all phase docs (`a`, `b`, `diag-*`); the
engine surfaces `.red`/`.yellow`/`.orange` drift, all as `.nudge` (phase drift is
...
TDD coverage:
  File 1 — PhaseScannerDocCoverageTests: the scanner reads `a`-tier and `diag-*` docs.
  File 2 — DisciplineEnginePhaseDriftSeverityTests: drift findings are nudge, not block;
  yellow drift is surfaced.

---

## Write to: MerlinTests/Unit/PhaseScannerDocCoverageTests.swift
...
xcodebuild -scheme MerlinTests test -destination 'platform=macOS' \
  ...
  -only-testing:MerlinTests/PhaseScannerDocCoverageTests \
...
## Commit
```
git add MerlinTests/Unit/PhaseScannerDocCoverageTests.swift \
  MerlinTests/Unit/DisciplineEnginePhaseDriftSeverityTests.swift \
  phases/phase-323a-phasescanner-doc-coverage-tests.md
git commit -m "Phase 323a — PhaseScanner doc-coverage & drift-severity tests (failing)"
```
