...
Forward-looking scope that is intentionally outside the current implementation roadmap. Items here are not gaps; they are deliberate deferrals captured so they are not lost. Each item names the domain it belongs to, why it is deferred today, and the conditions under which it should be reconsidered.

Architectural source of truth remains `architecture.md`. This document is a parking lot for capability ideas — not a commitment.
