...
KiCad integration is a first-class domain extension with deterministic completion gates. Active electronics behavior lives in `plugins/electronics` and routes through the workspace bus: raster/PDF schematic ingestion, KiCad CLI as primary authority, `LocalFreeRoutingBackend` for local FreeRouting-backed auto-route, optional hosted FreeRouting when configured, ERC/DRC/parity/SPICE/fab gates, required Gerbers, Excellon drills, BOM, pick-and-place, verification report artifacts, and vendor-native BOM/order workflows for JLCPCB / PCBWay / OSHPark / custom. Completion is evidence-gated: missing artifacts or failed gates block the workflow, and placeholder success responses are not accepted. High-stakes designs (hazardous energy, isolation, mission-critical) require explicit user sign-off; visual QA can block on presentation issues but cannot override electrical or fabrication gates. The electronics job panel shows backend health, progress, artifacts, diagnostics, approvals, and final reports for blocked or complete jobs.
